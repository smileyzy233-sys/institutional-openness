...
(default `true`). Domestic flows are removed by default; they remain only when
`row_policy.keep_domestic` is explicitly `true` (default `false`). Comparisons
ignore case and surrounding whitespace without changing original ISO values;
empty or missing endpoints are not domestic pairs. Overlaps count once in the
total deletions. Stata checks and removes both categories before OLS/PPML data
construction, including when consuming unchanged old input snapshots. Domestic raw
...
- any configured Gravity candidate, including trade-facilitation membership
  and agreement fields, `entry_*`, cultural/legal/colonial fields,
  `scaled_sci_2021`, `diplo_disagreement`, and configured derived candidates.
