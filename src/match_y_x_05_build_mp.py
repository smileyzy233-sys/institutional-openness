"""Left-join AMNE Y to raw_mp_score without adding trade-equation controls."""
...
def build_mp(y_data: pd.DataFrame, x_data: pd.DataFrame) -> pd.DataFrame:
    before = len(y_data)
    merged = y_data.merge(
        x_data[X_KEY + ["raw_mp_score"]],
        on=X_KEY,
        how="left",
    ...
    if len(merged) != before:
        raise AssertionError("MP Y-X merge changed the AMNE row count")
    ...
    if forbidden:
        raise AssertionError(f"MP Y-X output contains controls: {forbidden}")
    validate_unique(merged, Y_KEY, "MP Y-X output")
    return merged
...
def main() -> int:
    ...
    data = build_mp(
        prepare_y.prepare_y(root, specs, "mp", args.year),
        prepare_x.prepare_x(root, specs, args.year),
    )
    print(
        f"mp Y-X {args.year}: rows={len(data)}, "
