...
def completed_human_stage2(row: pd.Series | None) -> dict[str, Any] | None:
    ...
    record = {
        ...
        "raw_mp_weight": row.get("human_final_mp_weight"),
        "confidence": None,
    }
...
def valid_arbitration_stage2(row: pd.Series | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "final_mp_weight": normalized["final_mp_weight"],
...
def consensus_stage2(row: pd.Series) -> dict[str, Any]:
    ...
    if impact_type == "both":
        trade_weight, mp_weight = average_both_weights(
            row["model_a_trade_weight"],
            row["model_a_mp_weight"],
            row["model_b_trade_weight"],
            row["model_b_mp_weight"],
        )
        method = "dual_model_weight_mean"
    else:
        trade_weight, mp_weight = normalize_stage2_weights(impact_type, None, None)
...
def assert_final_consistent(final_df: pd.DataFrame, provisions: pd.DataFrame) -> None:
    ...
    assert pd.to_numeric(
        final_df.loc[both_mask, "final_mp_weight"],
        errors="coerce",
    ).gt(0).all()
    both_sum = (
        pd.to_numeric(final_df.loc[both_mask, "final_trade_weight"], errors="coerce")
        + pd.to_numeric(final_df.loc[both_mask, "final_mp_weight"], errors="coerce")
    )
    ...
    for impact_type, (trade_weight, mp_weight) in fixed_expectations.items():
        ...
        assert pd.to_numeric(
            final_df.loc[mask, "final_mp_weight"],
            errors="coerce",
        ).eq(mp_weight).all()
...
def run(*, allow_unresolved: bool = config.ALLOW_UNRESOLVED) -> None:
    ...
    if not comparison.empty:
        assert_impact_label_schema(comparison, "Stage 2 comparison")
    ...
    if not arbitration.empty:
        assert_impact_label_schema(arbitration, "Stage 2 arbitration")
    if not manual.empty:
        assert_impact_label_schema(manual, "Stage 2 manual review queue")
    ...
    for _, stage1_row in stage1_final.iterrows():
        ...
        base["effective_mp_weight"] = (
            0.0
            if int(base["final_is_institutional_opening"]) == 0
            else base["final_mp_weight"]
        )
