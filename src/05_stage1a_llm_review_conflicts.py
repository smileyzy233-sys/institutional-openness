...
def prompt_row(row: pd.Series) -> dict[str, Any]:
    ...
    for key in [
        ...
        "policy_area",
        "original_coding",
        "model_a_institutional_reason",
        "model_b_institutional_reason",
    ]:
...
def heuristic_arbitration_response(row: pd.Series) -> str:
    decision = heuristic_stage1a_decision(row)
    payload = {
        "final_is_institutional_opening": decision["is_institutional_opening"],
        "arbitration_reason": "Development-only deterministic Stage 1A arbitration heuristic.",
        "confidence": 0.8,
...
def complete_record(
    ...
    *,
    ...
    return {
        ...
        "final_is_institutional_opening": parsed.get("final_is_institutional_opening"),
        ...
        "prompt_version": config.STAGE1A_ARBITRATION_PROMPT_VERSION,
    ...
    for attempt in range(1, config.MAX_LLM_RETRIES + 1):
        ...
        try:
            ...
            try:
                ...
                normalized, status, message = validate_stage1a_arbitration_output(parsed)
                if status == "ok":
...
def empty_outputs() -> None:
    write_csv(pd.DataFrame(columns=ARBITRATION_COLUMNS), config.STAGE1A_ARBITRATION_RESULTS_PATH)
    write_csv(
        pd.DataFrame(columns=["provision_id", "review_context_hash", *HUMAN_FIELDS]),
        config.STAGE1A_MANUAL_REVIEW_QUEUE_PATH,
    )
...
def run(
    *,
    ...
    prompt_path: Path = config.STAGE1A_ARBITRATION_PROMPT_PATH,
    resume: bool = True,
    limit: int | None = None,
) -> None:
    ensure_directories()
    ...
    if not config.STAGE1A_CONFLICT_QUEUE_PATH.exists():
        raise FileNotFoundError(
            f"Stage 1A conflict queue not found: {config.STAGE1A_CONFLICT_QUEUE_PATH}"
        )
    queue = read_csv(config.STAGE1A_CONFLICT_QUEUE_PATH)
    if queue.empty:
        empty_outputs()
        print("Stage 1A conflict queue is empty; wrote empty arbitration/manual review files.")
        return
    ...
    if not existing.empty:
        ...
        mask = (
            ...
            & existing["prompt_version"].eq(config.STAGE1A_ARBITRATION_PROMPT_VERSION)
    ...
    run_id = make_run_id("stage1a_arbitration")
    rows = existing.to_dict("records")
    print(f"Stage 1A arbitration pending conflicts: {len(pending):,}")
    for index, (_, row) in enumerate(pending.iterrows(), start=1):
        ...
        if index == 1 or index % 25 == 0 or index == len(pending):
            print(
                f"Stage 1A arbitration: {index:,}/{len(pending):,} "
    ...
    write_csv(results[ARBITRATION_COLUMNS], config.STAGE1A_ARBITRATION_RESULTS_PATH)
    ...
    existing_manual = (
        read_csv(config.STAGE1A_MANUAL_REVIEW_QUEUE_PATH)
        if config.STAGE1A_MANUAL_REVIEW_QUEUE_PATH.exists()
        else pd.DataFrame()
    )
    manual_queue = merge_existing_manual_review(manual_queue, existing_manual, HUMAN_FIELDS)
    write_csv(manual_queue, config.STAGE1A_MANUAL_REVIEW_QUEUE_PATH)
    print(f"Wrote Stage 1A arbitration results to {config.STAGE1A_ARBITRATION_RESULTS_PATH}")
    print(
        f"Wrote {len(manual_queue):,} Stage 1A manual review rows to "
        f"{config.STAGE1A_MANUAL_REVIEW_QUEUE_PATH}"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Review Stage 1A dual-model conflicts.")
    parser.add_argument(
        "--provider",
        default=None,
        choices=[
    ...
    parser.add_argument("--prompt-path", type=Path, default=config.STAGE1A_ARBITRATION_PROMPT_PATH)
