...
def test_both_weight_average_does_not_trigger_arbitration():
    ...
    trade, mp = average_both_weights(0.7, 0.3, 0.5, 0.5)
    assert trade == 0.6
    assert mp == 0.4


def test_both_large_difference_still_averages():
    trade, mp = average_both_weights(0.9, 0.1, 0.1, 0.9)
    assert trade == 0.5
    assert mp == 0.5


def test_fixed_type_mapping():
    assert normalize_stage2_weights("trade", 0.8, 0.2) == (1.0, 0.0)
    assert normalize_stage2_weights("mp", 0.8, 0.2) == (0.0, 1.0)
