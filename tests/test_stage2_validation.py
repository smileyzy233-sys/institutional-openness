...
def test_stage2_trade_fixed_type_overrides_raw_weights():
    record, status, message = validate_stage2_output(
        {
            "provision_id": "P1",
            "impact_type": "trade",
            "raw_trade_weight": 0.8,
            "raw_mp_weight": 0.2,
            "confidence": 0.8,
        }
    )
    ...
    assert record["normalized_mp_weight"] == 0.0
...
def test_stage2_mp_fixed_weight():
    record, status, message = validate_stage2_output(
        {
            "provision_id": "P3",
            "impact_type": "mp",
    ...
    assert record["normalized_mp_weight"] == 1.0
